<template>
    ...
    <el-dialog title="选择用户" v-model="visibleDialog" style="width: 800px !important" :before-close="handleClose"
        ...
        <el-row :gutter="10">
                    ...
                    <el-col :span="24">
                        <el-form :model="qform" ref="queryRef" :inline="true">
                            <el-form-item label="用户名称" prop="description">
                                <el-input v-model="qform.description" placeholder="请输入用户名称" clearable
                                    style="width: 150px" size="default" @keyup.enter.native="handleQuery" />
                            </el-form-item>
                            <el-form-item>
                                <el-button type="primary" size="default" @click="handleQuery">搜索</el-button>
                            </el-form-item>
                        </el-form>
                        <el-table ref="refTable" :data="userList" v-loading="loading" border height="445px">
            ...
            <el-col :span="8">
                <p class="tip">已选中列表</p>
                <el-table ref="selectedTable" :data="checkedUsersList" border height="507px">
...
import { getUsers } from "@/workflow/api/workflow";
...
const getPageList = () => {
    loading.value = true;
    getUsers({ page: pageDto.value.page, pageSize: pageDto.value.pageSize, keyword: qform.value.description }).then((res) => {
        loading.value = false;
        const resp = res.data; total.value = resp.data?.total || 0;
        userList.value = (resp.data?.list || []).map((item) => {
            return {
                userId: item.id,
                userName: item.username,
                email: item.email || "",
                status: item.isFrozen ? 1 : 0
            };
        });
    }).catch(() => {
